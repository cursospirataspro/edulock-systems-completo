...
function fixture() {
    ...
    const db = {
        ...
        claimAndActivateLicenseAtomic: async data => { state.events.push(['activate', data]); return { ok: true, activationId: 'activation-a', license: { ...state.license, student_id: data.studentId }, maxDevices: 2, expiresAt: null, producerId: state.license.producer_id || null }; },
        createContentSession: async data => { state.events.push(['session', data]); return { id: data.id }; },
