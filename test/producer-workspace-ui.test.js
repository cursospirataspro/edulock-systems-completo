...
function harness() {
  ...
  const document={getElementById:node,createElement:()=>new Node(),querySelector:()=>null,querySelectorAll:selector=>selector==='[data-page]'?nav:selector==='[data-workspace-page]'?sections:[],body:new Node(),addEventListener(){},removeEventListener(){},activeElement:null};
  const context=vm.createContext({document,window:{addEventListener(){},EdulockTree:{buildTree:()=>({roots:[],orphans:[],nodes:new Map()}),descendantIds:()=>new Set(),canReparent:()=>true,createTreeView:()=>({render(){},toggle(){},isDragging:()=>false,openState:()=>new Set(),setOpen(){}})}},location:{origin:'https://edulock.example.invalid',hash:''},history:{replaceState(){},pushState(){}},sessionStorage:{setItem(){},getItem(){}},localStorage:{setItem(){},removeItem(){}},
