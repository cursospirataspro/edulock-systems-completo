...
function createProducerContent({ db, generatePublicCode, syncCollection, deleteProviderAsset } = {}) {
    ...
    async function remove(producerId, kind, id) {
        ...
        return transaction(producerId, async client => {
            ...
            return { ok: true, id, courseId: kind === 'course' ? id : (row.course_id || null) };
        }).then(async result => {
            // El borrado en el proveedor va DESPUES de confirmar la base: si algo falla aqui,
            // queda un archivo huerfano recuperable, nunca un borrado que no se pueda deshacer.
            if (typeof deleteProviderAsset !== 'function') return { ...result, providerFilesDeleted: false };
            try {
                const outcome = await deleteProviderAsset({ kind, producerId, id, courseId: result.courseId,
                    moduleId: kind === 'module' ? id : null, videoId: kind === 'video' ? id : null });
                return { ...result, providerFilesDeleted: outcome?.deleted === true,
                    ...(outcome?.deleted === true ? {} : { providerNote: outcome?.reason || null }) };
            } catch (error) {
                return { ...result, providerFilesDeleted: false,
                    providerWarning: 'Se eliminó de Edulock, pero no se pudo borrar en el servicio de video' +
                        (error?.message ? ': ' + String(error.message) : '') + '. Puedes borrarlo desde el panel del proveedor.' };
            }
        });
    }
...
function mountProducerContent(app, { db, requireProducer, generatePublicCode, service, syncCollection, deleteProviderAsset } = {}) {
