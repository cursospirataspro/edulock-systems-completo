...
function createProducerContent({ db, generatePublicCode, syncCollection } = {}) {
    ...
    async function updateVideo(producerId, id, input) {
        ...
        return transaction(producerId, async client => {
            ...
            try {
                ...
                // en su módulo actual. Así Edulock y el proveedor nunca quedan descoordinados en silencio.
                const now = typeof db.getCatalogById === 'function' ? await db.getCatalogById(id).catch(() => null) : null;
                const stillCurrent = !now || ((now.moduleId || null) === (moduleId || null) && (now.courseId || null) === (courseId || null));
                if (typeof db.setCollectionSyncPending === 'function') await db.setCollectionSyncPending(id, !stillCurrent);
                if (stillCurrent) return { ...dto, collectionSyncPending: false, providerWarning: null };
                return { ...dto, collectionSyncPending: true, providerWarning: 'La clase volvió a moverse mientras se sincronizaba; la colección se ajustará automáticamente.' };
            } catch (error) {
...
function mountProducerContent(app, { db, requireProducer, generatePublicCode, service, syncCollection } = {}) {
    ...
    service = service || createProducerContent({ db, generatePublicCode, syncCollection });
...
module.exports = { ensureSchema, createProducerContent, mountProducerContent, getPublicVideoPresentation, validatePatch, validateSettings, httpsUrl, embedOrigins, documentsPresent, legacyDocumentCount, courseDto, moduleDto, videoDto, contentError, DEFAULT_PRESENTATION, DEFAULT_SETTINGS };
