...
function createProducerContent({ db, generatePublicCode, syncCollection } = {}) {
    ...
    async function updateVideo(producerId, id, input) {
        ...
        return transaction(producerId, async client => {
            ...
            if (typeof syncCollection !== 'function') return { ...dto, collectionSyncPending: true, providerWarning: 'La colección de Bunny se sincronizará automáticamente.' };
            try {
                await syncCollection({ producerId, videoId: id, courseId, moduleId });
                if (typeof db.setCollectionSyncPending === 'function') await db.setCollectionSyncPending(id, false);
                return { ...dto, collectionSyncPending: false, providerWarning: null };
            } catch (error) {
                return { ...dto, collectionSyncPending: true, providerWarning: 'La clase se movió en Edulock. La colección de Bunny no se pudo actualizar ahora y se reintentará automáticamente' + (error?.message ? ': ' + String(error.message).slice(0, 160) : '.') };
            }
        });
    }
