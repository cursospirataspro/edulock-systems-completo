...
function videoDto(row) {
    return { videoId: row.video_id, courseId: row.course_id || null, moduleId: row.module_id || null, title: row.title, status: row.status,
        ...
        presentation: presentationDto(row.settings) };
}
...
function createProducerContent({ db, generatePublicCode } = {}) {
    ...
    async function projects(producerId) {
        identifier(producerId);
        const [courses, modules, videos] = await Promise.all([
            ...
            pool.query(`SELECT m.id,m.course_id,m.parent_id,m.name,m.sort_order,m.created_at,s.playlist_code FROM modules m JOIN courses c ON c.id=m.course_id AND c.producer_id=m.producer_id
            ...
            pool.query(`SELECT v.video_id,v.course_id,v.module_id,v.title,v.status,v.source_type,v.sort_order,v.public_code,v.uploaded_at,s.settings FROM catalog v
        ...
        return { projects: courses.rows.map(row => ({ ...courseDto(row), modules: modules.rows.filter(m => m.course_id === row.id).map(moduleDto), videos: videos.rows.filter(v => v.course_id === row.id).map(videoDto) })), unassignedVideos: videos.rows.filter(v => !v.course_id).map(videoDto) };
    }
    ...
    async function updateVideo(producerId, id, input) {
        ...
        return transaction(producerId, async client => {
            ...
            const result = (await client.query('UPDATE catalog SET title=$3,course_id=$4,module_id=$5,sort_order=$6 WHERE video_id=$1 AND producer_id=$2 RETURNING *', [id, producerId, patch.title ?? row.title, courseId, moduleId, patch.sortOrder ?? row.sort_order])).rows[0];
    ...
    async function remove(producerId, kind, id) {
        ...
        return transaction(producerId, async client => {
            ...
            return { ok: true, id, providerFilesDeleted: false };
        });
    }
    ...
    async function reorder(producerId, input) {
        inputObject(input, ['kind', 'courseId', 'ids']);
        ...
        return transaction(producerId, async client => {
            ...
            const table = input.kind === 'modules' ? 'modules' : 'catalog', key = input.kind === 'modules' ? 'id' : 'video_id';
            const rows = (await client.query(`SELECT ${key} AS id FROM ${table} WHERE course_id IS NOT DISTINCT FROM $1 AND producer_id=$2 FOR UPDATE`, [courseId, producerId])).rows;
            ...
            return { ok: true, kind: input.kind, courseId, count: ids.length };
        });
    }
...
function mountProducerContent(app, { db, requireProducer, generatePublicCode, service } = {}) {
    if (typeof requireProducer !== 'function') throw new TypeError('Producer authentication is required.');
    service = service || createProducerContent({ db, generatePublicCode });
    ...
    app.get('/playlist/:code', async (req, res) => {
        try {
            ...
            res.type('html').send(`<!doctype html><html lang="es"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(data.name)} — Edulock</title><style nonce="${nonce}">body{margin:0;background:#111116;color:#f6f6f8;font:16px system-ui}main{max-width:840px;margin:0 auto;padding:48px 24px}.brand{font-size:13px;letter-spacing:.15em;color:#f35b67}h1{font-size:32px;margin:8px 0}p{color:#b6b6c4;line-height:1.6;white-space:pre-wrap;overflow-wrap:anywhere}ul{list-style:none;padding:0}li{background:#1d1d26;border:1px solid #343442;border-radius:14px;display:flex;gap:20px;padding:20px;margin:16px 0}li div{min-width:0}li img{width:144px;height:88px;object-fit:cover;border-radius:8px}a{color:#ff6874;font-weight:650;text-decoration:none;overflow-wrap:anywhere}a:hover{text-decoration:underline}@media(max-width:520px){li{display:block}li img{width:100%;height:auto;margin-bottom:16px}}footer{border-top:1px solid #343442;padding-top:24px;margin-top:32px}</style><main><div class="brand">EDULOCK SYSTEMS</div><p>${escapeHtml(data.courseName)}</p><h1>${escapeHtml(data.name)}</h1><p>Selecciona una clase. Su reproducción requiere el acceso correspondiente y el reproductor de Edulock.</p><ul>${items || '<li>Todavía no hay clases publicadas en esta lista.</li>'}</ul><footer><a href="/download">Obtener el reproductor</a></footer></main></html>`);
        } catch (error) { if (!res.headersSent) contentError(res, error); }
    });
    return service;
}

module.exports = { ensureSchema, createProducerContent, mountProducerContent, getPublicVideoPresentation, validatePatch, validateSettings, httpsUrl, embedOrigins, documentsPresent, courseDto, moduleDto, videoDto, contentError, DEFAULT_PRESENTATION, DEFAULT_SETTINGS };
