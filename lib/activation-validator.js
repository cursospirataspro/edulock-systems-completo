...
function createActivationValidator({ db, jwtSecret, accessPolicy }) {
  return async function validate(body = {}) {
    ...
    if (videoId) {
      // La activación guardada equivale a una sesión de contenido de ESTA licencia: se autoriza
      // solo el curso de la licencia (identidad ≠ autorización; una licencia por sesión).
      const sessionClaims = { sub: student.id, deviceId, hasLicense: true, licenseId: license.id,
        courseId: license.course_id || undefined, allowedVideos: license.course_id ? [license.course_id] : [] };
      const context = await accessPolicy.authorizeVideo(sessionClaims, videoId, deviceId);
