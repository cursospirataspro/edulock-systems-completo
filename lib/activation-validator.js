...
function createActivationValidator({ db, jwtSecret, accessPolicy }) {
  return async function validate(body = {}) {
    ...
    if (expired(license.expires_at)) throw fail('Licencia expirada.', 'ACTIVATION_EXPIRED');
    ...
    if (license.producer_id) {
      ...
      if (!producer || !active(producer.active) || student.producerId !== license.producer_id) {
