...
function mountProducerBusiness(app, { db, requireProducer, requireAdmin, hashPassword, verifyPassword, issueProducerToken, secret, getPublicBase, mailConfigured = () => false }) {
    ...
    route('get', prefix + '/overview', requireProducer, async (req, res) => {
        ...
        const row = (await q(`SELECT
          ...
          (SELECT COUNT(DISTINCT student_id)::int FROM licenses WHERE producer_id=$1 AND student_id IS NOT NULL) customers,
        ...
        res.json({ projects: row.projects, videos: row.videos, licenses: row.licenses, students: row.customers, customers: row.customers, activeActivations: row.active_activations, openRequests: row.open_requests });
    });
