...
function installResourceRoutes(app, { service, multer, requireAccount, requireManager, deviceFor }) {
    ...
    app.post('/api/resources/upload', requireManager, uploadPdf, wrap(async (req, res) => res.status(201).json({ resource: await service.upload(req.user, req.body || {}, req.file) })));
