...
function createResourceService({ repository, storage, renderer, accessPolicy, secret, getProducer }) {
    ...
    return {
        ...
        async replaceFile(actor, id, input, file) {
            const resource = await ownedResource(actor, id);
            // Los documentos nuevos son solo enlaces: reemplazar archivo sirve únicamente para los PDF
            // históricos que ya viven en el servidor. Un recurso de enlace nunca se convierte en archivo.
            if (!resource.storageKey) fail('RESOURCE_REPLACE_LINK_FORBIDDEN', 'Este documento es un enlace externo. Los documentos nuevos se gestionan por enlace; solo los PDF ya alojados pueden reemplazar su archivo.', 409);
            const expectedVersion = version(input.expectedVersion);
