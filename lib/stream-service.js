...
const { confirmStillInModule } = require('./producer-content');

const STAGES = Object.freeze({
    'library-create': 'Creación de la biblioteca del curso',
    'library-verify': 'Verificación de la biblioteca',
    'library-activate': 'Activación de la clave de la biblioteca nueva',
    ...
    'video-create': 'Creación de la clase en el servicio de video',
    'video-collection': 'Asociación del video a la colección',
    'player-key': 'Preparación de la clave del reproductor',
    'video-transfer': 'Transferencia del archivo al servicio de video',
    'video-status': 'Consulta del estado del video',
    'video-delete': 'Borrado del video en el servicio de video',
    'collection-delete': 'Borrado de la colección del módulo',
    'library-delete': 'Borrado de la biblioteca del curso',
});
...
function normalizeVideoStatus(video) {
    ...
    return { status, phase: phases[status] || 'unknown', encodeProgress, ready, failed,
        retryable: status === 6,
        error: failed ? (status === 6 ? 'La subida al servicio de video no se completó.' : 'El servicio de video no pudo procesar el archivo.') : null };
}
    ...
    function send({ method, hostname, path, accessKey, body, filePath, fileSize, onProgress }) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const req = request({ method, hostname, path, headers, timeout: filePath ? 120000 : 30000 }, response => {
                ...
                response.on('error', () => finish(failure('BUNNY_CONNECTION_ERROR', 'La conexión con el servicio de video se interrumpió.', 502, true)));
                response.on('data', chunk => {
                    bytes += chunk.length;
                    if (bytes > 1024 * 1024) { response.destroy(); finish(failure('BUNNY_RESPONSE_TOO_LARGE', 'Respuesta del servicio de video no válida.')); }
                ...
                response.on('end', () => {
                    ...
                    if (response.statusCode < 200 || response.statusCode >= 300) {
                        ...
                        return finish(failure('BUNNY_HTTP_ERROR', `El servicio de video respondió HTTP ${response.statusCode}.`, 502, retryable,
                            { httpStatus: response.statusCode, provider: providerDetail(response.statusCode, text) }));
                    }
                    let result;
                    try { result = text ? JSON.parse(text) : {}; }
                    catch { return finish(failure('BUNNY_INVALID_JSON', 'El servicio de video devolvió una respuesta incompleta.', 502, true)); }
                    if (result?.success === false || result?.Success === false) {
                        return finish(failure('BUNNY_REJECTED', 'El servicio de video rechazó la operación.', 502, false, { provider: providerDetail(response.statusCode, text) }));
                    }
                    finish(null, result);
                });
            });
            req.on('error', () => finish(failure('BUNNY_CONNECTION_ERROR', 'No se pudo completar la conexión con el servicio de video.', 502, true)));
            req.on('timeout', () => finish(failure('BUNNY_TIMEOUT', 'El servicio de video no respondió dentro del plazo.', 504, true)));
            if (filePath) {
                let uploaded = 0;
                ...
                pipeline(source, meter, req).catch(() => finish(failure('UPLOAD_INTERRUPTED', 'La transferencia al servicio de video se interrumpió.', 502, true)));
...
function createStreamService({ db, getAccountKey, createKey, transport = createTransport(), logger = console, maxUploadBytes = 1024 ** 3, maxConcurrentUploads = 2, wait = ms => new Promise(resolve => setTimeout(resolve, ms)),
    // Empaquetado .edu. Cuando esta disponible, toda subida nueva se protege con
    // el contenedor propio en lugar de dejarse en HLS del proveedor. Se inyecta
    // desde server.js, que es donde viven las credenciales del almacenamiento.
    edu = null }) {
    ...
    async function accountKey() {
        ...
        if (!key) throw failure('BUNNY_NOT_CONFIGURED', 'Configura la cuenta del servicio de video antes de subir videos.', 503);
        return key;
    }
    ...
    async function findRemote(host, path, key, name, field) {
        ...
        for (let page = 1; page <= 100; page++) {
            ...
            if (!Array.isArray(items)) throw failure('BUNNY_INVALID_LIST', 'El servicio de video devolvió una lista no válida.');
    ...
    async function provision(resourceKey, remoteName, find, create, idOf) {
        ...
        if (found) {
            const id = idOf(found);
            if (!id) throw failure('BUNNY_MISSING_ID', 'El servicio de video no devolvió el identificador del recurso.');
        ...
        return db.withStreamLock(`course:${courseId}`, async () => {
            ...
            const createLibrary = async () => {
                ...
                const resource = await atStage('library-create', () => provision(`course:${courseId}`, name,
                    ...
                    lib => lib.Id));
                await db.setCourseBunnyLibrary(courseId, { libraryId: resource.id, libraryKey: resource.value?.ApiKey || null, pullZone: null });
                return resource.id;
            };
            let libraryId = saved?.libraryId, fresh = false;
            if (!libraryId) { libraryId = await createLibrary(); fresh = true; }
            ...
            let library;
            try { library = await readLibrary(); }
            catch (error) {
                // The saved library can belong to another Bunny account (the account key
                // changed). When the current key is valid but cannot see that library, the
                // course gets a fresh library in the current account and its modules get
                // new collections. The previous reference is archived, never deleted, and
                // the classes already published keep their original addresses.
                const status = Number(error?.httpStatus);
                if (!saved?.libraryId || ![401, 403, 404].includes(status)) throw error;
                await atStage('library-verify', () => transport.json('GET', ACCOUNT_HOST, '/videolibrary?page=1&perPage=1', key));
                warn('library-foreign', { courseId, libraryId: String(libraryId), status });
                if (typeof db.archiveCourseBunnyLibrary === 'function') await db.archiveCourseBunnyLibrary(courseId, { ...saved, reason: `HTTP ${status}` });
                await db.clearStreamResource(`course:${courseId}`);
                verifiedLibraries.delete(courseId);
                const modules = typeof db.getModulesByCourse === 'function' ? await db.getModulesByCourse(courseId) : [];
                for (const mod of modules) {
                    await db.setModuleBunnyCollection(mod.id, null);
                    await db.clearStreamResource(`module:${mod.id}`);
                    verifiedCollections.delete(mod.id);
                }
                libraryId = await createLibrary(); fresh = true;
                library = await readLibrary();
            }
            if (fresh) {
                // The Stream API rejects a just-created library key (HTTP 401) for a few
                // seconds. Wait until it is accepted so the first collection or upload
                // of the course does not fail; give up as retryable after ~1 minute.
                await atStage('library-activate', async () => {
                    for (let attempt = 1; ; attempt++) {
                        try { await transport.json('GET', STREAM_HOST, `/library/${libraryId}/collections?page=1&itemsPerPage=1`, library.ApiKey); return; }
                        catch (error) {
                            if (Number(error?.httpStatus) !== 401 || attempt >= 20) throw failure('BUNNY_LIBRARY_KEY_PENDING', 'La biblioteca nueva todavía no acepta su clave. Vuelve a intentarlo en un minuto.', 503, true, { stage: 'library-activate' });
                            warn('library-key-pending', { courseId, libraryId: String(libraryId), attempt });
                            await wait(3000);
                        }
                    }
                });
            ...
            if (managed && library.EnableDRM !== true) {
                ...
                library = await readLibrary();
                if (library.EnableDRM !== true) throw failure('BUNNY_DRM_UNCONFIRMED', 'El servicio de video todavía no confirmó la protección DRM de la biblioteca.', 503, true, { stage: 'library-protect' });
            }
            ...
            if (library.EnableDRM !== true) {
                ...
                if (pull.ZoneSecurityEnabled !== true || pull.ZoneSecurityIncludeHashRemoteIP === true ||
                    ...
                    throw failure('BUNNY_CDN_UNPROTECTED', 'El servicio de video todavía no confirmó la protección del CDN.', 503, true, { stage: 'cdn-verify' });
                }
            }
    ...
    function operationResult(op) {
        ...
        if (op.state === 'deleted') error = 'Este video se quitó del catálogo y esta operación ya no puede reanudarse.';
            ...
            error = 'No se confirmó la creación del recurso en el servicio de video.';
            action = 'Usa «Continuar seguimiento» o reintenta en unos minutos; no se creará un recurso duplicado.';
        } else if (op.state === 'error') {
            error = 'El servicio de video no pudo procesar el archivo.';
            action = 'Revisa el archivo y vuelve a subirlo como una clase nueva.';
        } else if (op.state === 'upload_failed') {
            error = 'La transferencia al servicio de video no se completó.';
            action = 'Selecciona el mismo archivo y usa «Reintentar envío».';
        } else if (rejected) {
            error = `Falló la etapa «${STAGES[stage] || 'preparación en el servicio de video'}».`;
            action = detail?.status && detail.status < 500
                ? 'El servicio de video rechazó la petición. Corrige la causa indicada y vuelve a seleccionar el archivo para reintentar.'
                : 'Vuelve a seleccionar el archivo para reintentar. El video no llegó al servicio de video.';
        }
    ...
    async function getOperationStatus({ operationId, actor }) {
        ...
        if (op.video_id && ['processing', 'ready'].includes(op.state)) {
            // Una clase .edu no vive en el servicio de streaming: su estado es el
            // que dejo la subida, no hay nada que preguntarle al proveedor.
            const entrada = await db.getCatalogById(op.video_id).catch(() => null);
            if (entrada?.sourceType === 'edu') return operationResult(op);
        ...
        try {
            return await db.withStreamLock(`upload:${operationId}`, async () => {
                ...
                try {
                    ...
                    // ── Camino .edu ─────────────────────────────────────────
                    // Si el contenedor propio esta disponible, la clase se protege
                    // con el en vez de subirse como HLS al proveedor. El panel no
                    // nota la diferencia: se escriben las mismas filas de operacion.
                    if (edu && await edu.disponible()) {
                        const videoId = op.video_id || crypto.randomUUID();
                        if (!op.video_id) op = await db.updateStreamOperation(operationId, { videoId }) || op;
                        await db.updateStreamOperation(operationId, { state: 'uploading', uploadPercent: 0, errorCode: null, errorDetail: null });
                        const hecho = await atStage('video-transfer', () => edu.empaquetarYSubir({
                            filePath, title: cleanTitle, videoId, courseId, moduleId,
                            producerId: course.producerId || null, operationId,
                            onProgress: percent => onProgress?.(percent),
                        }));
                        if (moduleId) await db.moveVideoToModule(videoId, moduleId).catch(() => {});
                        await db.updateStreamOperation(operationId, { state: 'ready', uploadPercent: 100, encodeProgress: 100, errorCode: null, errorDetail: null });
                        return { ...operationResult(await db.getStreamOperation(operationId)),
                            sourceType: 'edu', eduContentId: hecho.contentId, bytes: hecho.bytes };
                    }
                    ...
                    // Queda anotado en que biblioteca vive esta clase (R01).
                    if (typeof db.setVideoLibrary === 'function') {
                        try { await db.setVideoLibrary(videoId, library.libraryId); } catch (e) { warn('video-library-note-failed', { videoId, message: e.message }); }
                    }
    ...
    async function getVideoStatus({ videoId, actor }) {
        ...
        // La clase puede estar en una biblioteca anterior del curso (R01).
        const library = await libraryForVideo(entry.courseId, videoId);
        if (!library?.libraryId || !library.libraryKey) throw failure('BUNNY_LIBRARY_INCOMPLETE', 'La biblioteca de esta clase no está disponible.', 409);
        ...
        if (video.guid && video.guid !== videoId) throw failure('BUNNY_VIDEO_MISMATCH', 'El servicio de video devolvió otro video.');
    ...
    // Background repair for every producer: a module saved without its collection
    // (failed provisioning) gets it created without waiting for the next upload.
    // Failures are logged per module and retried on a later pass, never in a loop.
    /**
     * Coloca un video ya existente en la colección de su módulo actual (o fuera de toda colección
     * cuando moduleId es null). Reutiliza la actualización de video de Bunny (POST /library/{id}/videos/{guid}
     * con collectionId). No descarga ni vuelve a subir el archivo.
     */
    async function syncVideoCollection({ courseId, videoId, moduleId = null, actor }) {
        await authorizeCourse(courseId, actor);
        if (!UUID.test(String(videoId || ''))) throw failure('BUNNY_INVALID_VIDEO', 'Identificador de video no válido.', 400);
        const library = await ensureCourseLibrary({ courseId, actor });
        const own = await libraryForVideo(courseId, videoId);
        // Una clase de una biblioteca anterior no se mueve dentro de la nueva: sus
        // colecciones no existen alli y la operacion tocaria contenido equivocado (R01).
        if (own?.libraryId && String(own.libraryId) !== String(library.libraryId)) {
            throw failure('BUNNY_VIDEO_OTHER_LIBRARY', 'Esta clase pertenece a una biblioteca anterior del curso y no se puede reorganizar en la actual.', 409);
        }
        const collectionId = moduleId ? await ensureModuleCollection({ courseId, moduleId, actor }) : '';
        const remote = await atStage('video-collection', () => transport.json('GET', STREAM_HOST, `/library/${library.libraryId}/videos/${encodeURIComponent(videoId)}`, library.libraryKey));
        const current = remote?.collectionId ?? remote?.CollectionId ?? '';
        if (String(current || '') !== String(collectionId || '')) {
            await atStage('video-collection', () => transport.json('POST', STREAM_HOST, `/library/${library.libraryId}/videos/${encodeURIComponent(videoId)}`, library.libraryKey, { collectionId }));
        }
        return { videoId, collectionId: collectionId || null, changed: String(current || '') !== String(collectionId || '') };
    }

    /**
     * Biblioteca en la que vive una clase concreta. Si la clase tiene anotada su
     * biblioteca y no es la vigente del curso, se busca entre las que el curso
     * dejo de usar, con su propia clave. Asi una clase antigua nunca se consulta
     * ni se borra contra la biblioteca nueva (R01). Sin anotacion se usa la
     * vigente, que es donde esta mientras el curso no haya cambiado.
     */
    async function libraryForVideo(courseId, videoId) {
        const current = await db.getCourseBunny(courseId);
        const noted = typeof db.getVideoLibraryId === 'function' ? await db.getVideoLibraryId(videoId) : null;
        if (!noted || !current?.libraryId || String(noted) === String(current.libraryId)) return current;
        const previous = typeof db.getCoursePreviousLibraries === 'function' ? await db.getCoursePreviousLibraries(courseId) : [];
        const match = previous.find(entry => String(entry.libraryId) === String(noted));
        if (match && match.libraryKey) {
            return { ...current, libraryId: match.libraryId, libraryKey: match.libraryKey, pullZone: match.pullZone || null, archived: true };
        }
        // La clase dice estar en otra biblioteca de la que ya no se tiene la clave:
        // no se inventa nada ni se opera contra la biblioteca equivocada.
        return { ...current, libraryId: String(noted), libraryKey: null, unresolved: true };
    }

    /**
     * Describe QUE habria que borrar en el servicio de video, leyendo las filas
     * mientras todavia existen y comprobando la procedencia. Se llama ANTES del
     * borrado local; lo que devuelve es autosuficiente, asi que el borrado
     * externo posterior ya no depende de ninguna fila borrada (F02).
     *
     * La procedencia se comprueba para los tres tipos, videos incluidos (R02):
     * solo se borra un recurso que esta plataforma creo, identificado por el
     * registro que se guardo al crearlo. Nunca se borra por parecido de
     * identificadores ni por datos que venga a decir el cliente.
     */
    async function describeProviderAsset({ kind, courseId, moduleId = null, videoId = null, actor }) {
        await authorizeCourse(courseId, actor);
        const saved = await db.getCourseBunny(courseId);
        if (!saved?.libraryId) return { deletable: false, reason: 'sin biblioteca' };
        const base = { kind, courseId, moduleId, videoId, libraryId: String(saved.libraryId), libraryKey: saved.libraryKey || null };

        if (kind === 'video') {
            if (!UUID.test(String(videoId || ''))) return { deletable: false, reason: 'identificador no valido' };
            // La clase puede vivir en una biblioteca anterior del curso (R01).
            const own = await libraryForVideo(courseId, videoId);
            if (!own?.libraryId) return { deletable: false, reason: 'sin biblioteca' };
            if (!own.libraryKey) return { deletable: false, reason: 'sin clave de la biblioteca de esa clase' };
            base.libraryId = String(own.libraryId);
            base.libraryKey = own.libraryKey;
            // Procedencia: el video se creo desde una operacion de subida nuestra y
            // aquel registro apunta exactamente a este identificador remoto.
            const operation = typeof db.getStreamOperationByVideo === 'function' ? await db.getStreamOperationByVideo(videoId) : null;
            const owned = operation ? await db.getStreamResource('upload:' + operation.id) : null;
            if (!owned || String(owned.remote_id) !== String(videoId)) {
                return { deletable: false, reason: 'el video no lo creo esta plataforma' };
            }
            return Object.assign({}, base, { deletable: true, remoteId: String(videoId) });
        }

        if (kind === 'module') {
            const collectionId = typeof db.getModuleBunnyCollection === 'function' ? await db.getModuleBunnyCollection(moduleId) : null;
            if (!collectionId) return { deletable: false, reason: 'el modulo no tenia coleccion' };
            const owned = await db.getStreamResource('module:' + moduleId);
            if (!owned || String(owned.remote_id) !== String(collectionId)) return { deletable: false, reason: 'la coleccion no la creo esta plataforma' };
            if (!saved.libraryKey) return { deletable: false, reason: 'sin clave de la biblioteca' };
            return Object.assign({}, base, { deletable: true, remoteId: String(collectionId) });
        }

        if (kind === 'course') {
            const owned = await db.getStreamResource('course:' + courseId);
            if (!owned || String(owned.remote_id) !== String(saved.libraryId)) return { deletable: false, reason: 'la biblioteca no la creo esta plataforma' };
            return Object.assign({}, base, { deletable: true, remoteId: String(saved.libraryId) });
        }
        return { deletable: false, reason: 'tipo no soportado' };
    }

    /**
     * Ejecuta un borrado ya descrito. No lee ninguna fila del contenido borrado,
     * asi que funciona igual ahora que dentro de media hora tras un reinicio. Es
     * idempotente: si el recurso ya no esta, se da por hecho. Devuelve 'done' o
     * 'gone', o lanza para que la cola lo reintente.
     */
    async function executeProviderDeletion(descriptor) {
        const { kind, libraryId, libraryKey, remoteId, courseId, moduleId, videoId } = descriptor;
        const gone = error => Number(error?.httpStatus) === 404;
        if (kind === 'edu') {
            // Contenedor .edu: vive en el almacenamiento, no en el servicio de
            // streaming. libraryId lleva la zona (vacia = la configurada ahora) y
            // remoteId la ruta dentro de ella.
            if (!edu?.borrar) { warn('edu-delete-sin-soporte', { videoId }); return 'gone'; }
            const estado = await edu.borrar(libraryId || null, remoteId);
            warn('edu-deleted', { courseId, videoId, ruta: remoteId, estado });
            return estado;
        }
        if (kind === 'video') {
            try {
                await atStage('video-delete', () => transport.json('DELETE', STREAM_HOST,
                    '/library/' + libraryId + '/videos/' + encodeURIComponent(remoteId), libraryKey));
            } catch (error) { if (!gone(error)) throw error; return 'gone'; }
            warn('video-deleted', { courseId, videoId: remoteId, libraryId: String(libraryId) });
            return 'done';
        }
        if (kind === 'module') {
            let state = 'done';
            try {
                await atStage('collection-delete', () => transport.json('DELETE', STREAM_HOST,
                    '/library/' + libraryId + '/collections/' + encodeURIComponent(remoteId), libraryKey));
            } catch (error) { if (!gone(error)) throw error; state = 'gone'; }
            try { await db.clearStreamResource('module:' + moduleId); } catch {}
            verifiedCollections.delete(moduleId);
            warn('collection-deleted', { courseId, moduleId, libraryId: String(libraryId) });
            return state;
        }
        if (kind === 'course') {
            let state = 'done';
            const key = await accountKey();
            try {
                await atStage('library-delete', () => transport.json('DELETE', ACCOUNT_HOST,
                    '/videolibrary/' + encodeURIComponent(remoteId), key));
            } catch (error) { if (!gone(error)) throw error; state = 'gone'; }
            try { await db.clearStreamResource('course:' + courseId); } catch {}
            verifiedLibraries.delete(courseId);
            warn('library-deleted', { courseId, libraryId: String(remoteId) });
            return state;
        }
        throw failure('DELETE_KIND_UNSUPPORTED', 'Tipo de borrado no admitido.', 400);
    }

    /**
     * Vacia la cola de borrados pendientes. Se llama justo despues de borrar y
     * tambien de forma periodica y al arrancar, de modo que un fallo de red o un
     * reinicio no dejen archivos huerfanos en el servicio de video.
     */
    async function runProviderDeletions({ limit = 5 } = {}) {
        if (stopped || typeof db.claimProviderDeletions !== 'function') return { processed: 0 };
        let processed = 0, done = 0, gone = 0, failed = 0, retried = 0;
        const pending = await db.claimProviderDeletions(limit);
        for (const row of pending) {
            processed++;
            const descriptor = { kind: row.kind, libraryId: row.library_id, libraryKey: row.library_key,
                remoteId: row.remote_id, courseId: row.course_id, moduleId: row.module_id, videoId: row.video_id };
            try {
                const state = await executeProviderDeletion(descriptor);
                await db.finishProviderDeletion(row.id, state);
                if (state === 'gone') gone++; else done++;
            } catch (error) {
                const outcome = await db.retryOrGiveUpProviderDeletion(row.id, row.attempts, error?.message || String(error));
                if (outcome === 'failed') { failed++; warn('provider-delete-abandoned', { kind: row.kind, courseId: row.course_id }); }
                else retried++;
            }
        }
        return { processed, done, gone, failed, retried };
    }

    /**
     * Borra en el proveedor lo que esta plataforma creo para este curso.
     * `kind` es 'video', 'module' o 'course'. Solo borra recursos propios (los que figuran
     * en stream_resources con el mismo identificador remoto): nunca toca contenido ajeno
     * ni bibliotecas adoptadas. Devuelve que se borro y por que, sin lanzar cuando el
     * recurso ya no existe en el proveedor.
     */
    async function deleteProviderAsset({ kind, courseId, moduleId = null, videoId = null, actor }) {
        await authorizeCourse(courseId, actor);
        const saved = await db.getCourseBunny(courseId);
        if (!saved?.libraryId) return { deleted: false, reason: 'sin biblioteca' };
        const key = await accountKey();
        const gone = error => Number(error?.httpStatus) === 404;

        if (kind === 'video') {
            if (!UUID.test(String(videoId || ''))) return { deleted: false, reason: 'identificador no válido' };
            if (!saved.libraryKey) return { deleted: false, reason: 'sin clave de la biblioteca' };
            try {
                await atStage('video-delete', () => transport.json('DELETE', STREAM_HOST,
                    `/library/${saved.libraryId}/videos/${encodeURIComponent(videoId)}`, saved.libraryKey));
            } catch (error) { if (!gone(error)) throw error; return { deleted: false, reason: 'ya no estaba en el proveedor' }; }
            warn('video-deleted', { courseId, videoId, libraryId: String(saved.libraryId) });
            return { deleted: true };
        }

        if (kind === 'module') {
            const collectionId = typeof db.getModuleBunnyCollection === 'function' ? await db.getModuleBunnyCollection(moduleId) : null;
            if (!collectionId) return { deleted: false, reason: 'el módulo no tenía colección' };
            const owned = await db.getStreamResource(`module:${moduleId}`);
            if (!owned || String(owned.remote_id) !== String(collectionId)) return { deleted: false, reason: 'la colección no la creó esta plataforma' };
            if (!saved.libraryKey) return { deleted: false, reason: 'sin clave de la biblioteca' };
            try {
                await atStage('collection-delete', () => transport.json('DELETE', STREAM_HOST,
                    `/library/${saved.libraryId}/collections/${encodeURIComponent(collectionId)}`, saved.libraryKey));
            } catch (error) { if (!gone(error)) throw error; }
            await db.clearStreamResource(`module:${moduleId}`);
            verifiedCollections.delete(moduleId);
            warn('collection-deleted', { courseId, moduleId, libraryId: String(saved.libraryId) });
            return { deleted: true };
        }

        if (kind === 'course') {
            const owned = await db.getStreamResource(`course:${courseId}`);
            if (!owned || String(owned.remote_id) !== String(saved.libraryId)) return { deleted: false, reason: 'la biblioteca no la creó esta plataforma' };
            try {
                await atStage('library-delete', () => transport.json('DELETE', ACCOUNT_HOST,
                    `/videolibrary/${encodeURIComponent(saved.libraryId)}`, key));
            } catch (error) { if (!gone(error)) throw error; }
            await db.clearStreamResource(`course:${courseId}`);
            verifiedLibraries.delete(courseId);
            warn('library-deleted', { courseId, libraryId: String(saved.libraryId) });
            return { deleted: true };
        }
        return { deleted: false, reason: 'tipo no soportado' };
    }

    async function reconcileVideoCollections({ limit = 20 } = {}) {
        if (stopped || typeof db.getPendingCollectionSyncs !== 'function') return { synced: 0, errors: [] };
        const errors = []; let synced = 0;
        for (const { videoId, courseId, moduleId } of await db.getPendingCollectionSyncs(limit)) {
            if (stopped) break;
            try {
                await syncVideoCollection({ courseId, videoId, moduleId, actor: { admin: true } });
                // La reconciliación tampoco cierra una sincronización antigua: si la clase se movió otra vez
                // mientras el proveedor respondía (o no se puede comprobar), sigue pendiente para la próxima pasada.
                const check = await confirmStillInModule(db, videoId, courseId, moduleId);
                if (check.confirmed) { await db.setCollectionSyncPending(videoId, false); synced++; }
                else warn('video-collection-superseded', { videoId, courseId, moduleId, unknown: check.unknown === true });
            }
            catch (error) {
                errors.push({ videoId, code: error.code || 'STREAM_RECONCILE_ERROR', stage: error.stage || null });
                warn('video-collection-reconcile', { videoId, courseId, moduleId, ...describeError(error) });
                // A class published in a previous library of the course (account change) is not in
                // the current library: there is nothing to synchronise, so it must not retry forever.
                if (Number(error?.httpStatus) === 404 && error.stage === 'video-collection') { await db.setCollectionSyncPending(videoId, false); warn('video-collection-foreign', { videoId, courseId, moduleId }); }
            }
        }
        return { synced, errors };
    }

    async function reconcileCollections({ limit = 20 } = {}) {
        if (stopped || typeof db.getModulesWithoutCollection !== 'function') return { repaired: 0, errors: [] };
        const errors = []; let repaired = 0;
        for (const { moduleId, courseId } of await db.getModulesWithoutCollection(limit)) {
            if (stopped) break;
            try { await ensureModuleCollection({ courseId, moduleId, actor: { admin: true } }); repaired++; }
            catch (error) { errors.push({ moduleId, code: error.code || 'STREAM_RECONCILE_ERROR', stage: error.stage || null }); warn('collection-reconcile', { moduleId, courseId, ...describeError(error) }); }
        }
    ...
    return { ensureCourseLibrary, ensureModuleCollection, uploadVideo, getVideoStatus, getOperationStatus, reconcilePending, reconcileCollections,
        syncVideoCollection, reconcileVideoCollections, deleteProviderAsset,
        describeProviderAsset, executeProviderDeletion, runProviderDeletions, stop() { stopped = true; } };
}
