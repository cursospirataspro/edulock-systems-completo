...
function createStreamService({ db, getAccountKey, createKey, transport = createTransport(), logger = console, maxUploadBytes = 1024 ** 3, maxConcurrentUploads = 2 }) {
    ...
    async function reconcileVideoCollections({ limit = 20 } = {}) {
        ...
        for (const { videoId, courseId, moduleId } of await db.getPendingCollectionSyncs(limit)) {
            ...
            catch (error) { errors.push({ videoId, code: error.code || 'STREAM_RECONCILE_ERROR', stage: error.stage || null }); warn('video-collection-reconcile', { videoId, courseId, moduleId, ...describeError(error) }); }
        }
