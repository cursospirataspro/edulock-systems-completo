...
function createResourceRepository({ db } = {}) {
    ...
    async function resolve(client, kind, id) {
        ...
        // Igual que en el catálogo: la columna puede estar cifrada, así que se
        // lee con el conversor que entiende 'enc1:'. Un fallo de lectura se
        // registra en vez de convertirse en "no hay documentos".
        const parsedDocuments = require('../database-pg.js').parseDocuments(row.documents);
        if (parsedDocuments.reason) console.error('[recursos] documentos ilegibles en', kind, id, '·', parsedDocuments.reason);
        const documents = parsedDocuments.documents;
        if (!row.course_id) {
