...
const LICENSE_SQL = `
      ...
      WHERE l.student_id=$1 AND l.id=$4 AND (l.course_id=$3 OR l.course_id IS NULL)
      ORDER BY l.created_at DESC`;

/**
 * Identidad ≠ autorización. La cuenta (JWT) solo identifica al alumno; el contenido lo autoriza
 * la licencia de ESTA sesión (claims.licenseId / courseId / sid), nunca el conjunto de cursos
 * que el alumno haya comprado. Una sesión = un alumno + una licencia + un curso + un dispositivo.
 */
function createAccessPolicy({ db }) {
  async function requireOpenSession(claims) {
    if (!claims.sid || typeof db.getContentSession !== 'function') return null;
    const session = await db.getContentSession(claims.sid);
    if (!session || session.ended_at || session.student_id !== claims.sub || (claims.licenseId && session.license_id !== claims.licenseId)) {
      throw new AccessError('Tu sesión de contenido terminó. Ingresa tu licencia de nuevo.', 'SESSION_ENDED', 401);
    }
    return session;
  }
  ...
  async function hydrate(claims) {
    ...
    const contentSession = await requireOpenSession(claims);
    // Permisos de la sesión, no del alumno: solo el curso de la licencia activada (o el video del media token).
    const allowedVideos = Array.isArray(claims.allowedVideos)
      ? claims.allowedVideos.filter(v => typeof v === 'string' && v && v !== '*') : [];
    return { ...claims, email: student.email, allowedVideos, student, contentSession };
  }

  const sessionLicenseOf = user => (typeof user.licenseId === 'string' && user.licenseId ? user.licenseId : null);
  const sessionCourseOf = user => (typeof user.courseId === 'string' && user.courseId ? user.courseId : null);

  function resolveDevice(user, deviceId, message) {
    ...
    if (!device || device === 'unknown') throw new AccessError(message, 'LICENSE_REQUIRED');
    return device;
  }

  async function findSessionLicense(user, device, courseId, producerId) {
    const rows = (await db.pool.query(LICENSE_SQL, [user.sub, device, courseId || null, sessionLicenseOf(user)])).rows;
    return rows.find(row => {
      // Course licenses are permanent: only status, activation lease, device and producer state matter.
      ...
      if (row.producer_id && (!enabled(row.producer_active) || row.producer_id !== (producerId || null))) return false;
      return true;
    }) || null;
  }
  ...
  async function authorizeVideo(claims, videoId, deviceId, { requireActivation = true } = {}) {
    ...
    const licenseId = sessionLicenseOf(user), sessionCourse = sessionCourseOf(user);
    if (!hasVideoAccess(user, videoId, video.courseId)) {
      if (user.videoId && user.videoId !== videoId) throw new AccessError('No tienes acceso a este video.', 'VIDEO_FORBIDDEN');
      if (!video.courseId) throw new AccessError('No tienes acceso a este video.', 'VIDEO_FORBIDDEN');
      if (licenseId && sessionCourse && sessionCourse !== video.courseId) {
        throw new AccessError('Este video pertenece a otro curso. Cierra sesión e ingresa la licencia de ese curso.', 'COURSE_NOT_IN_SESSION');
      }
      throw new AccessError('Activa una licencia de este curso en tu dispositivo.', 'LICENSE_REQUIRED');
    }
    if (!licenseId) throw new AccessError('Ingresa la licencia del curso para esta sesión.', 'LICENSE_REQUIRED');
    if (sessionCourse && video.courseId && sessionCourse !== video.courseId) {
      throw new AccessError('Este video pertenece a otro curso. Cierra sesión e ingresa la licencia de ese curso.', 'COURSE_NOT_IN_SESSION');
    }
    const device = typeof deviceId === 'string' && deviceId ? deviceId : user.deviceId;
    if (user.deviceId && device && user.deviceId !== device) throw new AccessError('El dispositivo no corresponde a esta sesión.', 'DEVICE_MISMATCH');
    ...
    const license = await findSessionLicense(user, device, video.courseId, video.producerId);
  ...
  async function authorizeResource(claims, resource, deviceId) {
    ...
    const licenseId = sessionLicenseOf(user), sessionCourse = sessionCourseOf(user);
    if (user.videoId || user.sessionId || !licenseId || !user.allowedVideos.includes(resource.courseId)) {
      if (licenseId && sessionCourse && sessionCourse !== resource.courseId) {
        throw new AccessError('Este documento pertenece a otro curso. Cierra sesión e ingresa la licencia de ese curso.', 'COURSE_NOT_IN_SESSION');
      }
      throw new AccessError('Activa una licencia de este curso para abrir el documento.', 'LICENSE_REQUIRED');
    }
    const device = resolveDevice(user, deviceId, 'Activa la licencia en este dispositivo.');
    const license = await findSessionLicense(user, device, resource.courseId, resource.producerId);
    if (!license) throw new AccessError('Activa una licencia vigente de este curso en tu dispositivo.', 'LICENSE_REQUIRED');
